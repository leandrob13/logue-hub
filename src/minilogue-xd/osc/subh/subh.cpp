...
void OSC_INIT(uint32_t platform, uint32_t api) {
  ...
  oscillator.sub_gain = 1.0f;
  oscillator.semitone = 1;
  oscillator.undertone1 = 1;
  oscillator.undertone2 = 1;
...
uint16_t arp_undertone(void) {
    ...
    if (sub_arp.arp_type == down) {
        if (wave == 0.f && sub_arp.reset == 0 && roll) {
